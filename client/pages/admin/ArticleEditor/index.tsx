...
import { SelectField, InputField, TextAreaField, ButtonField, SwitchField } from '@/client/components/useValidateForm';
...
export default function ArticleEditor() {
	const { updateArticle, updateArticleLoading, createArticle, createArticleLoading, getAdminArticle, getAdminArticleLoading, articleState, setArticle } = useArticle();
	const { tagListState, getAdminList: getTagList } = useTagList();
	const { categoryListState, getAdminList: getCategoryList } = useCategoryList();
	const { getFormatUrl } = useLink();
	...
	const onSubmit = useCallback((values: any) => {
		...
		Modal.confirm({
			...
			onOk: () => window.open(getFormatUrl(`/u/${userState!.nickname}/a/${values.title}`))
		});
	}, [createArticle, getFormatUrl, updateArticle, userState]);

	const renderForm = useMemo(() => {
		if (categoryListState.list.length === 0 || tagListState.list.length === 0 || getAdminArticleLoading) {
			return <Skeleton />;
		}
		...
		return (
			<>
				<Formik validationSchema={schema} validateOnChange initialValues={data} onSubmit={onSubmit as any}>
					{
						({ values, setFieldValue }) => (
							<>
								<HeaderTitle
									title="编辑"
									aside={
										<ButtonField loading={updateArticleLoading || createArticleLoading} type="primary">
											提交
										</ButtonField>
									}
								/>
											...
											>
												{
													categoryListState.list.map((item) => (
														<Select.Option key={item.category_id.toString()} value={item.category_id}>
															{item.name}
														</Select.Option>
													))}
											</SelectField>
								...
								<SelectField
									...
									formItem={{
										label: <span>标签<WhiteSpace space={14} /></span>
									}}
								>
								...
								<TextAreaField
									...
									placeholder={'不超过150字'}
									formItem={{
										label: <span>摘要<WhiteSpace space={14} /></span>
									}}
								/>
								<SwitchField
									name="secret"
									checkedChildren={<><WhiteSpace space={4} />私密</>}
									unCheckedChildren={<><WhiteSpace space={4} />公开</>}
									formItem={{
										label: <span>状态<WhiteSpace space={14} /></span>
									}}
								/>
								<MarkdownEditor value={values.content} onChange={(v) => setFieldValue('content', v)} />
							</>
						)
					}
				</Formik>
	...
	}, [articleState, categoryListState.list, createArticleLoading, getAdminArticleLoading, onSubmit, tagListState.list, updateArticleLoading]);
