...
export type UploadItem = { url: string, status: 'pending' | 'done' | 'error'; };
...
export class Uploader {
	...
	public async uploadFiles(files: File[]) {
		...
		const uploadList: UploadItem[] = results.map(item => ({ url: '', status: 'pending' }));

		// 开始上传
		this.handler.start.map(fn => fn(uploadList));

		await PromiseEach(results.map(async (file, index) => {
			try {
