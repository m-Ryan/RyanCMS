...
import { markdownRender } from '@/client/utils/markdownRender';
...
export function MarkdownEditor(props: Props) {
  ...
  const onCallChange = useCallback((value: string) => {
    props.onChange(value);
  }, [props]);


  const onPluginCallChange = useCallback((payload: Parameters<PluginItemProps['onChange']>[0]) => {
  ...
  useLayoutEffect(() => {
    const isBlur = sectionRange.selectionStart === 0 && sectionRange.selectionEnd === 0;
    if (editor && !isBlur) {
      editor.focus();
      editor.setSelectionRange(sectionRange.selectionStart, sectionRange.selectionEnd);
      setSectionRange({
        selectionStart: 0,
        selectionEnd: 0
      });
          ...
          plugins.map((plugin, index) => {
            return (
              <a key={index} title={plugin.tip}>
                <plugin.content editor={ref.current!} onChange={onPluginCallChange} />
              </a>
            );
          })
        }
        <a key="全屏" title="全屏">
          {fullScreen ? <FullscreenExitOutlined onClick={() => setFullscreen(false)} /> : <FullscreenOutlined onClick={() => setFullscreen(true)} />}
        </a>
      </div>
    );
  ...
  const renderMarkdownPreview = useMemo(() => {
    return (
      <div className={`${styles['editor-view']} ry-table ${fullScreen ? '' : ' hidden-sm hidden-xs'}`} dangerouslySetInnerHTML={{ __html: markdownRender.render(value) }}></div>
    );
  }, [fullScreen, value]);
  ...
  }, [fullScreen, onCallChange, renderMarkdownPreview, renderTools, value]);
